  ...
  costmap_update_timeout_(300ms)
{
  ...
  declare_parameter("pause_execution", rclcpp::ParameterValue(false));
...
void ControllerServer::computeControl()
{
  ...
  try {
    ...
    while (rclcpp::ok()) {
      if (paused_) {
        RCLCPP_INFO_THROTTLE(get_logger(), get_clock(), 5000, "Controller is paused");
        publishZeroVelocity();  // Mantiene el robot detenido
  ...
  for (auto parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      if (name == "pause_execution") {
        paused_ = parameter.as_bool();
        RCLCPP_INFO(get_logger(), "Controller pause state set to: %s", paused_ ? "PAUSED" : "RUNNING");
